...
const FIELD_ALIASES: Record<string, string[]> = {
  ...
  title: ['title', 'job title', 'job_title', 'position', 'role', 'job role', 'jobtitle', 'job', 'designation', 'job position'],
};
...
const IGNORE_HEADERS = new Set([
  ...
  'intereststatus', 'interststatus', 'leadstatus', 'campaignname', 'campaign',
  ...
  'website', 'department', 'email', 'headline', 'location',
]);
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (body.rows && body.mapping) {
      ...
      // Assign to sequence if specified — randomize order and spread across days
      let sequenced = 0;
      if (body.sequence_id && count > 0) {
        ...
        if (seq) {
          ...
          if (steps.length > 0) {
            // Shuffle the contact IDs so different users don't process the same contacts first
            const shuffledIds = [...insertedIds].sort(() => Math.random() - 0.5);

            // Get user's daily limit and send window to spread queue items across days
            ...
            for (let i = 0; i < shuffledIds.length; i++) {
              ...
              // Calculate scheduled time: spread across multiple days
              const dayOffset = Math.floor(i / dailyLimit); // which day (0, 1, 2, ...)
              const positionInDay = i % dailyLimit; // position within that day
              const minuteOffset = Math.floor((positionInDay / dailyLimit) * windowMinutes) + Math.floor(Math.random() * 15); // spread + jitter
