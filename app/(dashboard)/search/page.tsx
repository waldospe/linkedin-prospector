...
export default function SearchPage() {
  ...
  return (
    <div className="space-y-6">
      {/* Header */}
      <div>
        <h1 className="text-2xl font-bold text-foreground">LinkedIn Search</h1>
      ...
      <div className="glass-card p-5 space-y-4">
        ...
        <div className="flex items-center gap-3">
          <button
            ...
            className={`text-xs font-medium px-3 py-1.5 rounded-lg transition-all ${!useUrl ? 'bg-blue-500/15 text-blue-400 border border-blue-500/20' : 'text-muted-foreground hover:text-foreground'}`}
          >
            Keyword Search
          </button>
          <button
          ...
          {useUrl ? (
            <div className="flex-1 relative">
              ...
              <input
                ...
                className="w-full h-11 input-field rounded-xl pl-10 pr-4 text-sm text-foreground placeholder:text-muted-foreground/40 focus:outline-none focus:border-blue-500/40 focus:ring-1 focus:ring-blue-500/20 transition-all"
              />
            </div>
          ) : (
            <div className="flex-1 relative">
          ...
          {results.length === 0 && !searching ? (
            ...
            <div className="space-y-2">
              {results.map((result, idx) => {
                ...
                return (
                  <div key={resultId} className="glass-card px-5 py-4 flex items-center gap-4 group hover:border-accent transition-all">
                    {/* Avatar */}
                    {result.profile_picture_url ? (
                      <img src={result.profile_picture_url} alt="" className="w-12 h-12 rounded-xl object-cover border border-border shrink-0" />
                    ) : (
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        <button
                          onClick={() => setViewProfile({ id: result.id || result.provider_id || '', name: displayName })}
                          className="text-sm font-medium text-foreground truncate hover:text-blue-400 transition-colors text-left"
                        >
                          {displayName}
                        </button>
                        {result.profile_url && (
                        ...
                        <>
                          {sequences.length > 0 && (
                            <select
                              ...
                              className="h-8 input-field text-[11px] rounded-lg px-2 focus:outline-none focus:border-blue-500/40 cursor-pointer max-w-[140px]"
                            >
              ...
              {cursor && (
                <div className="flex justify-center pt-2">
                  <button
                    ...
                    disabled={searching}
                    className="inline-flex items-center gap-2 text-sm font-medium px-5 py-2.5 rounded-xl bg-secondary border border-border text-foreground hover:bg-accent transition-all disabled:opacity-50"
                  >
      ...
      {viewProfile && (
        <ProfilePanel
          providerId={viewProfile.id}
          displayName={viewProfile.name}
